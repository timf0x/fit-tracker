export default {
  // ─── Common ───
  common: {
    save: 'Sauver',
    cancel: 'Annuler',
    delete: 'Supprimer',
    back: 'Retour',
    start: 'Commencer',
    close: 'Fermer',
    done: 'Terminé',
    loading: 'Chargement...',
    next: 'Continuer',
    confirm: 'Valider',
    error: 'Erreur',
    continue: 'Continuer',
    postpone: 'Reporter',
    modify: 'Modifier',
    all: 'Tous',
    exercises: 'exercices',
    sets: 'séries',
    reps: 'reps',
    minutes: 'minutes',
    search: 'Rechercher...',
    generating: 'Génération...',
    generate: 'Générer',
    comingSoon: 'Bientôt',
    comingSoonDesc: 'Cette fonctionnalité arrive bientôt.',
    letsGo: "C'est parti !",
    minAbbr: 'min',
    kgUnit: 'kg',
    cmUnit: 'cm',
    exosAbbr: 'exos',
    musclesUnit: 'muscles',
    currentLabel: 'Actuel',
    skip: 'Passer',
    validate: 'Valider',
    yes: 'Oui',
    no: 'Non',
    weekAbbr: 'S',
    deload: 'Décharge',
  },

  // ─── Tabs ───
  tabs: {
    home: 'Accueil',
    workouts: 'Entraînements',
    stats: 'Stats',
    settings: 'Réglages',
  },

  // ─── Equipment ───
  equipment: {
    fullGym: 'Salle complète',
    dumbbells: 'Haltères',
    bodyweight: 'Poids du corps',
    bodyWeight: 'Poids du corps',
    dumbbell: 'Haltères',
    barbell: 'Barre',
    cable: 'Câble',
    machine: 'Machine',
    kettlebell: 'Kettlebell',
    resistanceBand: 'Bande élastique',
    bands: 'Bande élastique',
    ezBar: 'Barre EZ',
    smithMachine: 'Smith machine',
    trapBar: 'Trap bar',
    other: 'Autre',
  },

  // ─── Body Parts (exercise categories) ───
  bodyParts: {
    chest: 'Pectoraux',
    upperBack: 'Haut du dos',
    lats: 'Dorsaux',
    lowerBack: 'Bas du dos',
    shoulders: 'Épaules',
    biceps: 'Biceps',
    triceps: 'Triceps',
    forearms: 'Avant-bras',
    quads: 'Quadriceps',
    hamstrings: 'Ischio-jambiers',
    glutes: 'Fessiers',
    calves: 'Mollets',
    abs: 'Abdominaux',
    obliques: 'Obliques',
    back: 'Dos',
    upperArms: 'Bras',
    upperLegs: 'Jambes',
    lowerLegs: 'Mollets',
    waist: 'Abdos',
    cardio: 'Cardio',
  },

  // ─── Body Parts (short labels for muscle bars) ───
  bodyPartsShort: {
    chest: 'Pectoraux',
    upperBack: 'Haut dos',
    lats: 'Dorsaux',
    lowerBack: 'Bas dos',
    shoulders: 'Épaules',
    biceps: 'Biceps',
    triceps: 'Triceps',
    forearms: 'Avant-bras',
    quads: 'Quadriceps',
    hamstrings: 'Ischio',
    glutes: 'Fessiers',
    calves: 'Mollets',
    abs: 'Abdos',
    obliques: 'Obliques',
  },

  // ─── Exercise Targets & Secondary Muscles ───
  targets: {
    lats: 'Dorsaux',
    upper_back: 'Haut du dos',
    middle_back: 'Milieu du dos',
    lower_back: 'Bas du dos',
    rear_delts: 'Deltoïdes postérieurs',
    lateral_delts: 'Deltoïdes latéraux',
    front_delts: 'Deltoïdes antérieurs',
    delts: 'Deltoïdes',
    traps: 'Trapèzes',
    pecs: 'Pectoraux',
    upper_chest: 'Pectoraux supérieurs',
    lower_chest: 'Pectoraux inférieurs',
    biceps: 'Biceps',
    brachialis: 'Brachial',
    triceps: 'Triceps',
    forearm_flexors: 'Fléchisseurs avant-bras',
    forearm_extensors: 'Extenseurs avant-bras',
    brachioradialis: 'Brachio-radial',
    grip: 'Grip',
    quads: 'Quadriceps',
    hamstrings: 'Ischio-jambiers',
    glutes: 'Fessiers',
    gastrocnemius: 'Gastrocnémien',
    soleus: 'Soléaire',
    calves: 'Mollets',
    abs: 'Abdominaux',
    lower_abs: 'Abdominaux inférieurs',
    obliques: 'Obliques',
    core_stability: 'Stabilité du tronc',
    // Secondary muscles
    latissimus_dorsi: 'Grand dorsal',
    shoulders: 'Épaules',
    forearms: 'Avant-bras',
    rotator_cuff: 'Coiffe des rotateurs',
    chest: 'Pectoraux',
    deltoids: 'Deltoïdes',
    core: 'Gainage',
    hip_flexors: 'Fléchisseurs de hanche',
    quadriceps: 'Quadriceps',
    ankles: 'Chevilles',
    cardiovascular: 'Cardiovasculaire',
    full_body: 'Corps entier',
  },

  // ─── Volume Zones ───
  zones: {
    belowMv: 'Sous MV',
    maintenance: 'Maintien',
    growth: 'Croissance',
    overload: 'Surcharge',
    danger: 'Danger',
    optimal: 'Optimal',
    high: 'Élevé',
    excess: 'Excès',
    shortBelowMv: 'Sous MV',
    shortMv: 'MV',
    shortMev: 'MEV',
    shortMav: 'MAV',
    shortMrv: 'MRV+',
  },

  // ─── Home Screen ───
  home: {
    greeting: {
      morning: 'BON MATIN',
      afternoon: 'BON APRÈS-MIDI',
      evening: 'BONSOIR',
    },
    stats: {
      recovery: 'RÉCUP',
      steps: 'PAS',
    },
    weekly: {
      title: 'CETTE SEMAINE',
      streak: 'sem',
      goalReached: 'objectif atteint !',
      days: ['L', 'M', 'M', 'J', 'V', 'S', 'D'],
      sessions: 'séances',
      pr: 'PR',
      volumeHebdo: 'VOLUME HEBDO',
    },
    friends: {
      title: 'amis',
      recent: 'RÉCENT',
      viewAll: 'Voir tout',
    },
    recommended: {
      title: 'Recommandés',
      viewAll: 'Voir tout',
    },
    workoutCard: {
      mins: 'mins',
      kcal: 'kcal',
    },
    achievement: {
      almostThere: 'Presque là !',
      finalStretch: 'Dernière ligne droite',
      keepGoing: '%{pct}% — continue comme ça',
      complete: '%{pct}% accompli',
    },
  },

  // ─── Workouts Tab ───
  workouts: {
    header: 'MES ENTRAÎNEMENTS',
    title: 'Entraînements',
    myWorkouts: 'Mes entraînements',
    noCustom: 'Aucun entraînement personnalisé',
    noCustomDesc: 'Créez votre premier entraînement personnalisé',
    createWorkout: 'Créer un entraînement',
    presets: 'Modèles',
    custom: 'Personnalisé',
    exercises: 'exercices',
    filters: {
      level: 'Niveau',
      focus: 'Focus',
      equipment: 'Équipement',
      duration: 'Durée',
    },
    deleteTitle: 'Supprimer ce workout ?',
    deleteMessage: 'Cette action est irréversible.',
  },

  // ─── Workout Create ───
  workoutCreate: {
    title: 'Nouvel entraînement',
    save: 'Sauver',
    namePlaceholder: 'Nom de l\'entraînement',
    iconSection: 'ICÔNE',
    exercisesSection: 'EXERCICES',
    addExercise: 'AJOUTER UN EXERCICE',
    addExercisesModal: 'Ajouter des exercices',
    searchPlaceholder: 'Rechercher un exercice...',
    exerciseCount: '%{count} exercices',
    selectedCount: '%{count} exercice sélectionné',
    selectedCountPlural: '%{count} exercices sélectionnés',
    addToWorkout: 'Ajouter à l\'entraînement',
    sets: 'Séries',
    reps: 'Répétitions',
    weight: 'Poids (kg)',
    rest: 'Repos (sec)',
    timePerSet: 'Temps/set (s)',
    errorNoName: 'Entrez un nom pour l\'entraînement',
    errorNoExercise: 'Ajoutez au moins un exercice',
    errorCreate: 'Impossible de créer l\'entraînement',
    icons: {
      push: 'Poussée',
      pull: 'Tirage',
      legs: 'Jambes',
      core: 'Abdos',
      cardio: 'Cardio',
      fullBody: 'Corps entier',
      upper: 'Haut',
      lower: 'Bas',
    },
    focus: {
      all: 'Tous',
      chest: 'Pectoraux',
      back: 'Dos',
      shoulders: 'Épaules',
      legs: 'Jambes',
      arms: 'Bras',
      waist: 'Abdos',
      cardio: 'Cardio',
    },
  },

  // ─── Workout Detail ───
  workoutDetail: {
    notFound: 'Entraînement introuvable',
    addExercisesModal: 'Ajouter des exercices',
    searchPlaceholder: 'Rechercher...',
    addToWorkout: 'Ajouter à l\'entraînement',
    removeExercise: 'Retirer cet exercice',
    addExercise: 'AJOUTER UN EXERCICE',
    deleteWorkout: 'Supprimer cet entraînement',
    sets: 'Séries',
    reps: 'Reps',
    weight: 'Charge (kg)',
    rest: 'Repos (s)',
    timePerSet: 'Temps/set (s)',
    rename: 'Renommer',
    delete: 'Supprimer',
    renameTitle: 'Renommer le workout',
    renameLabel: 'Nom du workout',
    deleteConfirmTitle: 'Supprimer ce workout ?',
    deleteConfirmMessage: 'Cette action est irréversible.',
    overloadBump: 'Progression : charge augmentée',
    overloadDrop: 'Charge réduite — consolide avant de progresser',
    overloadHold: 'Ajuste selon ton ressenti',
    alreadyTrainedTitle: 'Déjà une séance aujourd\'hui',
    alreadyTrainedMessage: 'Tu as déjà terminé une séance aujourd\'hui. La récupération est essentielle pour la progression.',
  },

  // ─── Workout Generate ───
  workoutGenerate: {
    title: 'Générer une séance',
    equipmentSection: 'ÉQUIPEMENT',
    durationSection: 'DURÉE',
    generate: 'Générer',
    generating: 'Génération...',
    startSession: 'Commencer',
    letsGo: 'C\'est parti !',
    alreadyTrainedTitle: 'Déjà une séance aujourd\'hui',
    alreadyTrainedMessage: 'Tu as déjà terminé une séance aujourd\'hui. Veux-tu quand même en commencer une autre ?',
    equipment: {
      fullGym: 'Salle',
      dumbbells: 'Haltères',
      bodyweight: 'Poids du corps',
    },
    muscleGroups: {
      push: 'POUSSÉE',
      pull: 'TIRAGE',
      legs: 'JAMBES',
      core: 'TRONC',
    },
    sessionLabels: {
      push: 'Poussée',
      pull: 'Tirage',
      legs: 'Jambes',
      upper: 'Haut du corps',
      lower: 'Bas du corps',
      fullBody: 'Corps entier',
      rest: 'Repos',
      default: 'Entraînement',
    },
    summaryMuscles: '%{count} muscle',
    summaryMusclesPlural: '%{count} muscles',
    summaryFormat: '%{muscles} · ~%{sets} séries · ~%{minutes} min',
    reviewFormat: '%{exercises} exos · %{sets} séries · ~%{minutes}min',
  },

  // ─── Workout Session ───
  workoutSession: {
    phases: {
      prepare: 'PRÉPARATION',
      exercise: 'EXERCICE',
      rest: 'REPOS',
      break: 'PAUSE',
      finished: 'TERMINÉ',
    },
    paused: 'EN PAUSE',
    setOf: 'SÉRIE %{current} SUR %{total}',
    rightSide: 'DROITE ›',
    leftSide: '‹ GAUCHE',
    overloadBump: '↑ Charge augmentée',
    overloadDrop: '↓ Charge réduite',
    viewExercises: 'VOIR LES EXERCICES',
    completedOf: '%{done} / %{total} terminés',
    inProgress: 'EN COURS',
    // Finished screen
    finishedBadge: 'TERMINÉ',
    finishedTitle: 'Entraînement terminé !',
    duration: 'Durée',
    completedSets: 'Séries complétées',
    totalVolume: 'Volume total',
    exercisesLabel: 'Exercices',
    finish: 'Terminer',
    personalRecords: 'RECORDS PERSONNELS',
    prWeight: 'Poids',
    prVolume: 'Volume',
    prReps: 'Reps',
    prBefore: 'avant',
    prNew: 'NOUVEAU',
    exerciseReview: 'DÉTAILS DES EXERCICES',
    // Stop modal
    stopTitle: 'Arrêter l\'entraînement ?',
    stopMessage: 'Que souhaitez-vous faire de cette séance ?',
    stopSave: 'Sauvegarder & Quitter',
    stopDiscard: 'Quitter sans sauvegarder',
    // Sound modal
    soundTitle: 'Son & Voix',
    soundEffects: 'Effets sonores',
    soundEffectsDesc: 'Bips, ticks, alertes',
    voiceAnnouncements: 'Annonces vocales',
    voiceAnnouncementsDesc: 'Phases, exercices, décompte',
    volumeLabel: 'VOLUME',
    // Exercise info modal
    about: 'À propos',
    guide: 'Guide',
    targetMuscle: 'Muscle cible',
    muscleGroup: 'Groupe musculaire',
    equipmentLabel: 'Équipement',
    type: 'Type',
    unilateral: 'Unilatéral',
    bilateral: 'Bilatéral',
    secondaryMuscles: 'Muscles secondaires',
    formTips: 'CONSEILS DE FORME',
    // Post-session feedback
    feedbackTitle: 'TON RESSENTI',
    feedbackPump: 'Congestion',
    feedbackPumpLow: 'Faible',
    feedbackPumpMed: 'Bonne',
    feedbackPumpHigh: 'Énorme',
    feedbackSoreness: 'Courbatures',
    feedbackSorenessLow: 'Aucune',
    feedbackSorenessMed: 'Modérées',
    feedbackSorenessHigh: 'Fortes',
    feedbackPerformance: 'Performance',
    feedbackPerfLow: 'En baisse',
    feedbackPerfMed: 'Stable',
    feedbackPerfHigh: 'En hausse',
    feedbackJointPain: 'Douleurs articulaires',
    feedbackNudgeOverreached: 'Surmenage détecté — pense à décharger',
    feedbackNudgeUnder: 'Tu peux pousser plus — ajoute du volume',
    feedbackNudgeOptimal: 'Stimulus optimal — continue comme ça !',
    feedbackNudgeJoint: 'Attention aux articulations — adapte tes mouvements',
    // RIR (Reps In Reserve)
    rirLabel: 'RIR',
    repsLabel: 'REPS',
    kgLabel: 'KG',
    exerciseDetailFormat: '%{completedSets}/%{totalSets} séries · %{reps} reps',
    exerciseDetailFormatKg: '%{completedSets}/%{totalSets} séries · %{reps} reps · %{weight} kg',
    rirTooltip: 'Reps en réserve — combien de reps de plus aurais-tu pu faire ?',
    rir0: 'Échec',
    rir1: 'Dur',
    rir2: 'Optimal',
    rir3: 'Modéré',
    rir4: 'Facile',
    // Unilateral labels
    uniTag: 'UNI',
    rightAbbr: 'D',
    leftAbbr: 'G',
    // Post-session: training load
    trainingLoad: 'Charge',
    trainingLoadUnit: 'UA',
    // Post-session: volume impact
    volumeImpact: 'IMPACT VOLUME HEBDO',
    setsAdded: '+%{count} séries cette séance',
    // Post-session: recovery forecast
    recoveryForecast: 'RÉCUPÉRATION ESTIMÉE',
    recoveryHours: '~%{hours}h',
    readyAt: 'Prêt %{time}',
    // Post-session: feedback transparency
    feedbackTransparencyMaintains: 'Ton feedback maintient le volume actuel.',
    feedbackTransparencyReduces: 'Ton feedback réduira le volume la semaine prochaine.',
    feedbackTransparencyIncreases: 'Ton feedback pourrait augmenter le volume.',
    feedbackTransparencyJoint: 'Des exercices alternatifs seront suggérés.',
    // Post-session: celebration subtitles
    celebrationBeast: 'Énorme séance — tu es une machine !',
    celebrationPR: 'Progression confirmée — continue comme ça.',
    celebrationGood: 'Bon travail — la constance paie.',
    celebrationShort: 'Mieux vaut ça que rien — reviens plus fort.',
    // Post-session: exercise detail collapse
    exerciseDetailToggle: 'DÉTAIL DES EXERCICES',
  },

  // ─── Program Onboarding ───
  programOnboarding: {
    goal: {
      title: 'Ton objectif',
      subtitle: 'Choisis ce qui te motive le plus',
      hypertrophy: 'Hypertrophie',
      hypertrophyDesc: 'Prendre du muscle, augmenter le volume',
      strength: 'Force',
      strengthDesc: 'Devenir plus fort, soulever plus lourd',
      recomposition: 'Recomposition',
      recompositionDesc: 'Perdre du gras, maintenir le muscle',
    },
    experience: {
      title: 'Ton expérience',
      subtitle: 'Cela influence le volume et la complexité du programme',
      beginner: 'Débutant',
      beginnerDesc: 'Moins de 1 an d\'entraînement régulier',
      intermediate: 'Intermédiaire',
      intermediateDesc: '1 à 3 ans d\'entraînement structuré',
      advanced: 'Avancé',
      advancedDesc: 'Plus de 3 ans, maîtrise des mouvements',
      trainingYears: '~%{count} an(s) d\'entraînement',
      trainingYearsLabel: 'Années d\'entraînement régulier',
    },
    frequency: {
      title: 'Fréquence',
      subtitle: 'Combien de jours par semaine peux-tu t\'entraîner ?',
      days: '%{count} jours',
    },
    equipment: {
      title: 'Ton équipement',
      subtitle: 'On adapte les exercices à ton setup',
      fullGym: 'Salle complète',
      fullGymDesc: 'Barres, haltères, câbles, machines',
      homeDumbbell: 'Home gym',
      homeDumbbellDesc: 'Haltères, élastiques, kettlebell',
      bodyweight: 'Poids du corps',
      bodyweightDesc: 'Aucun matériel, exercices au poids du corps',
    },
    limitations: {
      title: 'Zones sensibles',
      subtitle: 'On adaptera les exercices pour protéger tes articulations',
      shoulder: 'Épaule',
      knee: 'Genou',
      lowerBack: 'Bas du dos',
      hip: 'Hanche',
      elbow: 'Coude',
      wrist: 'Poignet',
    },
    measurements: {
      title: 'Ton profil',
      subtitle: 'Indispensable pour estimer tes charges de départ',
      name: 'Prénom',
      namePlaceholder: 'Ton prénom',
      sex: 'Sexe *',
      male: 'Homme',
      female: 'Femme',
      weight: 'Poids (kg) *',
      weightHint: 'Estime tes charges de départ',
      height: 'Taille (cm)',
      heightHint: 'Personnalise la sélection d\'exercices',
      age: 'Âge',
      ageHint: 'Adapte la récupération et les temps de repos',
      required: '* Champs obligatoires',
    },
    priority: {
      title: 'Muscles prioritaires',
      subtitle: 'Choisis 0 à 2 muscles à développer en priorité',
    },
    confirmation: {
      title: 'Ton programme',
      subtitle: 'Vérifie tes paramètres avant de générer',
      objective: 'Objectif',
      experience: 'Expérience',
      frequency: 'Fréquence',
      equipment: 'Équipement',
      sex: 'Sexe',
      weight: 'Poids',
      priorities: 'Priorités',
      limitations: 'Limitations',
      trainingYears: 'Années d\'entraînement',
      noLimitation: 'Aucune',
      daysPerWeek: '%{count} jours/sem',
      yearsCount: '%{count} an(s)',
      generate: 'Générer mon programme',
    },
  },

  // ─── Program Overview ───
  // ─── Program Labels (day names, split names) ───
  programLabels: {
    // Day labels
    upperA: 'Haut A',
    upperB: 'Haut B',
    lowerA: 'Bas A',
    lowerB: 'Bas B',
    fullBodyA: 'Corps entier A',
    fullBodyB: 'Corps entier B',
    fullBodyC: 'Corps entier C',
    fullBodyPump: 'Pump global',
    pushA: 'Poussée A',
    pushB: 'Poussée B',
    pullA: 'Tirage A',
    pullB: 'Tirage B',
    legsA: 'Jambes A',
    legsB: 'Jambes B',
    // Split names
    splitFullBody: 'Corps entier',
    splitUpperLower: 'Haut/Bas',
    splitPPL: 'Poussée/Tirage/Jambes',
    // Program name template
    programName: '%{split} – %{weeks} semaines',
    // Muscle suffix
    andMoreMuscles: '+%{count} autres',
  },

  programOverview: {
    thisWeek: 'CETTE SEMAINE',
    weekLabel: 'SEMAINE %{week}',
    deloadWeek: 'Semaine de décharge',
    deloadDesc: 'Volume réduit pour la récupération et la supercompensation.',
    completed: 'Programme terminé !',
    completedSessions: '%{count} séances complétées',
    newProgram: 'Nouveau programme',
    weekSummary: '%{done}/%{total} séances · %{sets} séries',
    modifyProgram: 'Modifier le programme',
    quitProgram: 'Quitter le programme',
    startSession: 'Commencer la séance',
    quitConfirmTitle: 'Quitter le programme ?',
    quitConfirmMessage: 'Ta progression sera perdue. Tu pourras toujours en créer un nouveau.',
    quitConfirmButton: 'Quitter',
    modifyConfirmTitle: 'Modifier le programme ?',
    modifyConfirmMessage: 'Ton programme actuel sera remplacé. Ta progression sera perdue.',
    alreadyTrainedTitle: 'Déjà une séance aujourd\'hui',
    alreadyTrainedMessage: 'Tu as déjà terminé une séance aujourd\'hui. La récupération est essentielle pour la progression.',
    programme: 'PROGRAMME',
    weekProgress: 'Semaine %{current}/%{total}',
    rirTarget: 'RIR %{rir}',
    rirFailure: 'Échec',
    volumeUp: '+%{delta} séries vs S%{prev}',
    volumeDown: '%{delta} séries vs S%{prev}',
    volumeSame: 'Volume identique',
    deloadVolume: 'Volume réduit',
    // RIR meter phases
    phaseAdaptation: 'Adaptation',
    phaseAccumulation: 'Accumulation',
    phaseIntensification: 'Intensification',
    phasePeak: 'Peak',
    phaseFailure: 'Échec musculaire',
    phaseRecovery: 'Récupération',
    // RIR tooltip
    rirTooltip: 'RIR = Répétitions en réserve. Cette semaine : garde %{rir} reps en réserve sur chaque série.',
    rirTooltipFailure: 'RIR = Répétitions en réserve. Cette semaine : pousse chaque série jusqu\'à l\'échec.',
    rirTooltipDeload: 'Semaine de récupération. Charge légère, technique parfaite.',
    // Smart nudges
    nudgeAdaptation: 'Semaine d\'adaptation — concentre-toi sur la technique et le ressenti.',
    nudgeAccumulation: 'Le volume augmente — les progrès se construisent ici.',
    nudgeIntensification: 'Pousse plus fort — moins de répétitions en réserve.',
    nudgePeak: 'Dernière ligne droite avant le deload — donne tout.',
    nudgeDeload: 'Volume réduit pour la supercompensation. Profite de la récupération.',
    // Volume snapshot
    nMoreMuscles: '+%{count} autres',
  },

  // ─── Program Day ───
  programDay: {
    done: 'Fait',
    today: 'Aujourd\'hui',
    planned: 'Planifié',
    exercisesCount: 'exercices',
    setsCount: 'séries',
    minutes: 'minutes',
    weekLabel: 'Semaine %{week}',
    compounds: 'COMPOSÉS',
    isolation: 'ISOLATION',
    lastTime: 'Dernière fois',
    overloadBump: 'Progression : charge augmentée à %{weight}kg',
    overloadDrop: 'Charge réduite — consolide avant de progresser',
    overloadHold: 'Même charge — continue de progresser en reps',
    suggestion: 'Suggestion : essaie %{suggestion}',
    adjustTip: 'Valeurs suggérées — ajuste selon ton ressenti',
    sets: 'Séries',
    repsMax: 'Reps max',
    minReps: 'min: %{min}',
    weight: 'Charge (kg)',
    rest: 'Repos (s)',
    timePerSet: 'Temps/set (s)',
    reset: 'Réinitialiser',
    deloadBanner: 'Semaine de décharge : volume réduit pour favoriser la récupération et la supercompensation.',
    estimatedWeightTip: 'Les charges sont estimées à partir de ton poids de corps. Ajuste selon ton ressenti.',
    sessionDone: 'Séance terminée',
    startSession: 'Commencer',
    sessionFormat: '~%{duration} min · %{sets} séries',
    catchUp: 'Rattraper cette séance',
    catchUpDesc: 'Séance planifiée précédemment',
    // Overload suggestions (from programGenerator)
    overloadReduce: 'Réduis à %{weight}kg',
    overloadBumpReps: '%{weight}kg (reviens à %{reps} reps)',
    overloadPlusRep: '+1 rep (objectif : %{reps})',
  },

  // ─── Stats Screen ───
  stats: {
    header: 'STATISTIQUES',
    title: 'Statistiques',
    noData: 'Aucune donnée',
    noDataDesc: 'Complète ta première séance pour voir tes statistiques ici.',
    viewWorkouts: 'Voir les workouts',
    sessions: 'séances',
    duration: 'durée',
    volume: 'volume',
    personalRecords: 'RECORDS PERSONNELS',
    prCount: '%{count} PR',
    viewAll: 'Voir tous',
    prWeight: 'Poids',
    prVolume: 'Volume',
    prReps: 'Reps',
    noPrThisPeriod: 'Aucun record cette période',
    keepPushing: 'Continuez à pousser !',
    balance: 'Équilibre',
    wellBalanced: 'Bon équilibre',
    imbalanced: 'Déséquilibré',
    weeklyVolume: 'Volume Hebdo',
    volumeFormat: '%{sets} séries · %{muscles} muscles',
    recentSessions: 'Dernières séances',
    bodyweight: 'PDC',
    muscleAbbr: {
      chest: 'Poit',
      upperBack: 'Dos',
      lats: 'Dors',
      lowerBack: 'Bdos',
      shoulders: 'Épau',
      biceps: 'Bice',
      triceps: 'Tric',
      forearms: 'Avan',
      quads: 'Quad',
      hamstrings: 'Isch',
      glutes: 'Fess',
      calves: 'Moll',
      abs: 'Abdo',
      obliques: 'Obli',
    },
    radarGroups: {
      legs: 'Jamb',
      back: 'Dos',
      abs: 'Abdo',
      forearms: 'Avan',
      biceps: 'Bice',
      triceps: 'Tric',
      calves: 'Moll',
      shoulders: 'Épau',
      chest: 'Poit',
    },
  },

  // ─── Recovery Screen ───
  recovery: {
    title: 'Récupération',
    score: 'SCORE',
    excellent: 'Excellent',
    partial: 'Partiel',
    restNeeded: 'Repos conseillé',
    front: 'Face',
    back: 'Dos',
    weeklyVolume: 'VOLUME HEBDO',
    status: 'Statut',
    lastSession: 'Dernière séance',
    weeklySets: 'Séries (sem.)',
    muscleStatus: 'ÉTAT DES MUSCLES',
    fresh: 'frais',
    fatigued: 'fatigués',
    undertrained: 'sous-entraînés',
    scoreTitle: 'Score de Récupération',
    scoreExplanation: 'Ce score indique l\'état de récupération global de vos muscles, basé sur vos dernières séances.',
    scoreExcellent: 'Vos muscles sont bien récupérés. Idéal pour un entraînement intensif.',
    scorePartial: 'Récupération en cours. Entraînez les muscles frais uniquement.',
    scoreRest: 'La plupart des muscles sont fatigués. Privilégiez le repos ou une séance légère.',
    yourScore: 'Votre score actuel',
    recoveryProgress: 'Progression de récupération',
    hoursRemaining: '~%{hours}h restantes',
    hoursProgress: '%{since}h / %{total}h',
    setsCount: '%{count} séries',
    viewVolumeHistory: 'Voir volume & historique',
    // Training recommendations
    recRest: 'Beaucoup de muscles sont fatigués — un jour de repos serait idéal.',
    recFresh: '%{muscles} sont frais — idéal pour une séance %{type}.',
    recUntrained: "%{muscles} n'ont pas été entraînés récemment.",
    recUntrainedSingle: "%{muscles} n'a pas été entraîné récemment.",
    recGeneral: 'Bonne récupération générale — choisis ton entraînement !',
    recLegs: 'jambes',
    // Time since
    timeNever: 'Jamais',
    timeLessThan1h: 'Il y a < 1h',
    timeHours: 'Il y a %{hours}h',
    timeDay: 'Il y a 1 jour',
    timeDays: 'Il y a %{days} jours',
  },

  // ─── Volume Screen ───
  volume: {
    title: 'Volume Hebdo',
    totalSets: 'séries',
    activeMuscles: 'muscles',
    volumePerWeek: 'VOLUME / SEMAINE',
    exercisesThisWeek: 'EXERCICES CETTE SEMAINE',
    noExerciseThisWeek: 'Aucun exercice cette semaine',
    landmarksSectionTitle: 'REPÈRES',
    currentWeek: 'Actu.',
    vsWeek: 'vs sem.',
    landmarks: {
      title: 'Repères de Volume',
      description: 'Le volume (séries par semaine) détermine vos gains musculaires. Ces repères vous aident à optimiser votre entraînement.',
      mv: 'Volume de Maintien',
      mvDesc: 'Minimum pour conserver vos gains sans progresser.',
      mev: 'Volume Minimum Efficace',
      mevDesc: 'Seuil à partir duquel vous commencez à progresser.',
      mav: 'Volume Adaptatif Max',
      mavDesc: 'Zone optimale pour la croissance musculaire. Visez cette plage !',
      mrv: 'Volume Max Récupérable',
      mrvDesc: 'Au-delà, vous risquez le surentraînement et la régression.',
    },
    trends: {
      title: 'TENDANCES',
      description: 'Les flèches comparent la semaine sélectionnée à la précédente.',
      volumeUp: 'Volume en hausse',
      volumeDown: 'Volume en baisse',
    },
    progression: {
      title: 'PROGRESSION IDÉALE',
    },
    zoneAdvice: {
      belowMv: 'Volume insuffisant — vous risquez de perdre vos gains',
      mvMev: 'Volume de maintien — augmentez pour progresser',
      mevMav: 'Zone optimale — continuez sur cette lancée !',
      mavMrv: 'Volume élevé — surveillez votre récupération',
      aboveMrv: 'Surentraînement probable — réduisez le volume',
    },
  },

  // ─── Steps Screen ───
  steps: {
    title: 'Pas',
    stepsUnit: 'pas',
    timeFilters: {
      week: 'Semaine',
      month: 'Mois',
      year: 'Année',
    },
    dayAbbr: ['D', 'L', 'M', 'M', 'J', 'V', 'S'],
    average: 'moyenne',
    record: 'record',
    goals: 'objectifs',
    total: 'total',
    successRate: 'taux de réussite',
    unavailable: 'Le podomètre n\'est pas disponible sur cet appareil. Les données de pas ne peuvent pas être récupérées.',
  },

  // ─── Trophies Screen ───
  trophies: {
    title: 'Trophées',
    summary: 'Résumé',
    unlocked: 'DÉBLOQUÉS',
    locked: 'À débloquer',
    allCategories: 'Toutes',
    points: 'points',
    pts: 'pts',
    ptsTo: 'pts →',
    badges: 'badges',
    level: 'NIVEAU',
    nextLevel: 'Prochain niveau',
    progress: 'Progression',
    secret: 'Secret',
    closeTo: 'Presque débloqué',
    unlockedOn: 'Débloqué le %{date}',
    recentAchievements: 'DERNIERS EXPLOITS',
    nextGoals: 'PROCHAINS OBJECTIFS',
    noUnlocks: 'Complète une séance pour débloquer des badges',
    noNextGoals: 'Continue à t\'entraîner pour progresser',
    celebration: {
      continue: 'Continuer',
      next: 'Suivant',
      newBadge: 'Nouveau',
      badgeUnlocked: 'BADGE DÉBLOQUÉ',
    },
  },

  // ─── Exercise Detail ───
  exercise: {
    periods: {
      '3m': '3 mois',
      '6m': '6 mois',
      '1y': '1 an',
    },
    notFound: 'Exercice introuvable',
    noData: 'Aucune donnée',
    addToNext: 'Ajoute cet exercice à ton prochain workout !',
    weightChart: 'Évolution du poids',
    volumeChart: 'Évolution du volume',
    summary: 'Résumé',
    bestWeight: 'Meilleur poids',
    bestVolume: 'Meilleur volume',
    totalSessions: 'Séances totales',
    lastSession: 'Dernière séance',
    history: 'HISTORIQUE',
    set: 'Série',
    bodyweight: 'PDC',
  },

  // ─── Settings Screen ───
  settings: {
    header: 'RÉGLAGES',
    title: 'Réglages',
    editProfile: 'Modifier',
    configureProfile: 'Configurer le profil',
    memberSince: 'Depuis le %{date}',
    daysPerWeekShort: '%{n}j/sem',

    // Sound & Voice
    soundSection: 'Son & Voix',
    soundEffects: 'Effets sonores',
    soundEffectsDesc: 'Bips, ticks, alertes',
    voiceAnnouncements: 'Annonces vocales',
    voiceAnnouncementsDesc: 'Phases, exercices, décompte',
    volumeLabel: 'VOLUME',

    // Preferences
    prefsSection: 'Préférences',
    units: 'Unités',
    unitsKg: 'kg',
    unitsLbs: 'lbs',
    language: 'Langue',
    languageFr: 'Français',
    languageEn: 'English',
    keepAwake: 'Écran actif en séance',
    keepAwakeDesc: "L'écran reste allumé pendant l'entraînement",

    // Data
    dataSection: 'Données',
    clearHistory: "Réinitialiser l'historique",
    clearHistoryDesc: 'Supprimer toutes les séances enregistrées',
    clearHistoryConfirmTitle: 'Réinitialiser ?',
    clearHistoryConfirmMsg: 'Toutes tes séances seront supprimées définitivement. Cette action est irréversible.',
    clearHistoryConfirmBtn: 'Supprimer tout',
    clearHistoryCancel: 'Annuler',

    // About
    aboutSection: 'À propos',
    version: 'Version',
    contact: 'Nous contacter',

    // Profile edit screen
    profileTitle: 'Mon Profil',
    profileChangePhoto: 'Changer la photo',
    profileName: 'Prénom',
    profileNamePlaceholder: 'Ton prénom',
    profileUsername: 'Pseudo',
    profileUsernamePlaceholder: '@pseudo',
    profileEmail: 'E-mail',
    profileEmailPlaceholder: 'ton@email.com',
    profileEmailInvalid: 'Adresse e-mail invalide',
    profileSex: 'Sexe',
    profileWeight: 'Poids',
    profileHeight: 'Taille',
    profileAge: 'Âge',
    profileExperience: 'Niveau',
    profileGoal: 'Objectif',
    profileEquipment: 'Équipement',
    profileEquipmentDetail: 'Personnalise ton équipement disponible',
    profileEquipmentAlwaysOn: 'toujours actif',
    profileDays: 'Jours par semaine',
    profileLimitations: 'Limitations',
    profilePriority: 'Muscles prioritaires',
    profileSave: 'Enregistrer',
    profileWarning: 'Modifier ces paramètres peut affecter ton programme actif',
    profileIdentity: 'IDENTITÉ',
    profileBody: 'MENSURATIONS',
    profileTraining: 'ENTRAÎNEMENT',
    profileStatsSessions: 'séances',
    profileStatsVolume: 'tonnes',
    profileStatsStreak: 'sem. streak',
    male: 'Homme',
    female: 'Femme',
    cm: 'cm',
    years: 'ans',
  },

  // ─── Prefs (legacy compat) ───
  prefs: {
    header: 'RÉGLAGES',
    title: 'Préférences',
    myProfile: 'Mon profil',
    myTrophies: 'Mes Trophées',
    preferences: 'Préférences',
    language: 'Langue',
    notifications: 'Notifications',
    units: 'Unités (kg/lbs)',
    aboutSection: 'À propos',
    about: 'À propos',
    contact: 'Nous contacter',
    version: 'Version 1.0.0',
  },

  // ─── Levels ───
  levels: {
    beginner: 'Débutant',
    intermediate: 'Intermédiaire',
    advanced: 'Avancé',
    all: 'Tous niveaux',
  },

  // ─── Workout Types ───
  workoutTypes: {
    express: 'Express',
    complete: 'Complet',
  },

  // ─── Active Program Card (Home) ───
  activeProgram: {
    createTitle: 'Crée ton programme',
    createSubtitle: 'PPL · Haut/Bas · Full Body · 4-6 sem',
    createCta: 'C\'est parti',
    programLabel: 'PROGRAMME',
    completed: 'Programme terminé !',
    sessions: 'séances',
    weeks: 'semaines',
    split: 'split',
    newProgram: 'Nouveau programme',
    start: 'Commencer',
    sessionDone: 'Séance terminée',
    moreExercises: '+%{count} exercices',
    alreadyTrainedTitle: 'Déjà une séance aujourd\'hui',
    alreadyTrainedMessage: 'Tu as déjà terminé une séance aujourd\'hui. La récupération est essentielle pour la progression.',
    splitLabels: {
      ppl: 'PPL',
      upperLower: 'Haut/Bas',
      fullBody: 'Full Body',
    },
  },

  // ─── Recovery Card (Home) ───
  recoveryCard: {
    label: 'RÉCUP',
    nudgeGood: 'Bonne récupération — prêt à pousser',
    nudgeModerate: 'Récup partielle — reste attentif',
    nudgeLow: 'Fatigue élevée — envisage un deload',
  },

  // ─── Smart Suggestion Card ───
  smartSuggestion: {
    label: 'SÉANCE SUGGÉRÉE',
    firstSession: 'Première séance',
    firstSessionDesc: 'Génère une séance en choisissant tes muscles, ou crée un programme personnalisé.',
    generateSession: 'Générer une séance',
    letsGo: 'C\'est parti !',
    createManual: 'Créer manuellement',
    program: 'Programme',
    rest: 'Repos',
    generateButton: 'Générer la séance',
    summaryFormat: '%{muscles} muscles · ~%{sets} séries · ~%{minutes} min',
  },

  // ─── Session Insights ───
  sessionInsights: {
    weeklyVolume: 'VOLUME HEBDO',
    staleWeightTip: '%{weight}kg depuis %{sessions} séances — essaie %{next}kg',
  },

  // ─── Deload Detection ───
  deload: {
    urgentMessage: 'Fatigue accumulée sur %{muscles}. Deload recommandé cette semaine.',
    warningMessage: '%{muscles} au-dessus du MRV depuis %{weeks} semaines. Pense à une décharge.',
  },

  // ─── Readiness Check ───
  readiness: {
    title: 'Check de préparation',
    sleep: 'Sommeil',
    energy: 'Énergie',
    stress: 'Stress',
    soreness: 'Courbatures',
    submit: 'Commencer la séance',
    sleepBad: 'Mauvais',
    sleepOk: 'Correct',
    sleepGood: 'Excellent',
    energyLow: 'Basse',
    energyMid: 'Normale',
    energyHigh: 'Élevée',
    stressHigh: 'Élevé',
    stressMid: 'Modéré',
    stressLow: 'Faible',
    sorenessHigh: 'Fortes',
    sorenessMid: 'Modérées',
    sorenessLow: 'Aucune',
    score: 'Score',
    levelPeak: 'Au top',
    levelGood: 'Bonne forme',
    levelModerate: 'Forme moyenne',
    levelLow: 'Fatigué',
    nudgePeak: 'Pousse tes limites !',
    nudgeGood: 'Séance normale.',
    nudgeModerate: 'Séance adaptée automatiquement.',
    nudgeLow: 'Séance très allégée.',
    adjustmentPreview: 'Volume %{volume} · Repos %{rest} · RIR %{rir}',
    recLow: 'Ton corps a besoin de repos. Séance légère conseillée.',
    recHigh: 'Tu es en forme ! Pousse un peu plus.',
  },

  // ─── Exercise Swap ───
  exerciseSwap: {
    title: "Remplacer l'exercice",
    noAlternatives: 'Aucune alternative disponible',
    choose: 'Choisir',
  },

  // ─── Session Feedback (Program) ───
  sessionFeedbackModal: {
    title: 'Comment était cette séance ?',
    pump: 'Pump',
    pumpLow: 'Faible',
    pumpMed: 'Bon',
    pumpHigh: 'Énorme',
    soreness: 'Courbatures attendues',
    sorenessLow: 'Aucune',
    sorenessMed: 'Modérées',
    sorenessHigh: 'Fortes',
    performance: 'Performance',
    perfLow: 'Baisse',
    perfMed: 'Stable',
    perfHigh: 'Hausse',
    jointPain: 'Inconfort articulaire',
  },

  // ─── Body Map ───
  bodyMap: {
    front: 'Avant',
    back: 'Arrière',
    fatigued: 'Fatigué',
    fresh: 'Frais',
    undertrained: 'Sous-entraîné',
  },

  // ─── Scheduling (Onboarding) ───
  scheduling: {
    preferredDaysTitle: 'Quels jours tu t\'entraînes ?',
    preferredDaysSubtitle: 'Sélectionne exactement %{count} jours',
    dayLabels: ['L', 'M', 'M', 'J', 'V', 'S', 'D'],
    dayLabelsFull: ['Lundi', 'Mardi', 'Mercredi', 'Jeudi', 'Vendredi', 'Samedi', 'Dimanche'],
    startDateTitle: 'Quand commences-tu ?',
    startDateSubtitle: 'Tu pourras toujours ajuster plus tard',
    today: 'Aujourd\'hui',
    todayDesc: 'Commence dès maintenant',
    nextMonday: 'Lundi prochain',
    nextMondayDesc: 'Commence la semaine prochaine',
    pickDate: 'Choisir une date',
    pickDateDesc: 'Sélectionne un jour précis',
    // Confirmation
    confirmDays: 'Jours d\'entraînement',
    confirmStart: 'Début',
    daysFormat: '%{days}',
    partialWeekNudge: 'En commençant aujourd\'hui, ta première semaine sera partielle. Ton programme s\'adapte automatiquement.',
  },

  // ─── Calendar Screen ───
  calendar: {
    title: 'CALENDRIER',
    weekView: 'Semaine',
    monthView: 'Mois',
    weekOf: 'Sem. du %{date}',
    // Day content
    restDay: 'Repos programmé',
    nothingPlanned: 'Rien de prévu',
    longRest: 'Longue pause — tes muscles seront tous frais',
    startSession: 'Commencer la séance',
    nextSession: 'Prochaine séance',
    todaySession: 'Séance prévue aujourd\'hui',
    readinessLabel: 'Tes muscles sont à %{pct}% — prêt',
    readinessLow: 'Attention : %{muscles} encore fatigués',
    projectionFresh: 'Projection : %{muscles} frais',
    // Month summary
    monthSessions: '%{count} séances',
    monthVolume: '%{volume}',
    monthPRs: '%{count} PR',
    // Smart nudges (home card)
    thisWeek: 'CETTE SEMAINE',
    todayNudge: 'Séance %{name} prévue aujourd\'hui',
    nextNudge: 'Prochaine : %{day} — %{name}',
    restNudge: 'Jour de repos — %{count} muscles en récupération',
    // Week summary card
    weekSummary: 'RÉSUMÉ DE LA SEMAINE',
    volumeByMuscle: 'Volume par muscle',
    mesocycleProgress: 'Mésocycle',
    weekProgress: 'Sem. %{n}/%{total}',
    // Day content — enriched states
    musclesTrained: 'Muscles entraînés',
    muscleReadiness: 'État des muscles',
    recoveryStatus: 'Récupération',
    musclesFresh: '%{count} muscles frais',
    musclesRecovering: '%{count} en récupération',
    musclesFatigued: '%{count} fatigués',
    readyTomorrow: '%{muscles} seront frais demain',
    inDays: 'Dans %{count}j',
    restDayTitle: 'Jour de repos',
    recoveryProjection: 'Projection récupération',
    fresh: 'frais',
    recovering: 'récupération en cours',
    fatigued: 'fatigué',
    // Month summary — enriched
    avgDuration: 'moy.',
    topMuscles: 'Top muscles',
    prs: 'PR',
    sessions: 'séances',
    volume: 'volume',
    // Week summary — additional
    noTraining: 'Pas encore d\'entraînement cette semaine',
    setsCount: '%{count} séries',
    prBadge: '%{count} PR',
  },

  // ─── Active Program Card — Schedule Additions ───
  activeProgram_schedule: {
    nextSessionDate: 'Prochaine séance : %{date} — %{name}',
    todayScheduled: 'Séance prévue aujourd\'hui',
    noSessionToday: 'Prochaine séance',
  },

  // ─── Program Overview — Schedule Additions ───
  programOverview_schedule: {
    scheduledDate: '%{date}',
    nextSessionNudge: 'Prochaine séance : %{day}',
  },

  // ─── Missed Day Intelligence ───
  missedDay: {
    // Card title
    title: 'Séance manquée',
    titleMultiple: '%{count} séances manquées',
    daysSince: '%{count}j sans entraînement',

    // Resolution options
    doMissed: 'Faire cette séance',
    doMissedDesc: 'Rattraper la séance manquée aujourd\'hui',
    skipContinue: 'Passer et continuer',
    skipContinueDesc: 'Ignorer la séance manquée, passer à la suivante',
    skipDeloadDesc: 'Semaine de décharge — pas de souci, le repos fait partie du plan',
    skipMultipleDesc: 'Trop de retard — on repart de la prochaine séance prévue',
    merge: 'Fusionner les muscles',
    mergeDesc: 'Ajouter les muscles manqués à la prochaine séance (+%{sets} séries)',
    reschedule: 'Reprogrammer la semaine',
    rescheduleDesc: 'Décaler les séances restantes à partir d\'aujourd\'hui',

    // Nudges
    nudgeSingle: 'Une séance en retard — pas de panique, on s\'adapte',
    nudgeMultiple: 'Plusieurs séances manquées — reprenons intelligemment',
    nudgeLongBreak: 'Longue pause détectée — tes muscles sont frais, c\'est le moment',
    nudgeDeload: 'Semaine de décharge — manquer une séance n\'a aucun impact',
    nudgeUrgent: 'Plusieurs jours sans entraînement — choisis comment reprendre',

    // Badges
    recommended: 'Recommandé',
    missedLabel: 'Manqué',
    skippedLabel: 'Passé',
    mergedLabel: 'Fusionné',

    // Recovery context
    musclesReady: 'Muscles frais et prêts',
    musclesRecovering: 'Certains muscles encore en récupération',
  },

  // ─── Priority Muscle Groups (Onboarding) ───
  priorityGroups: {
    chest: 'Pectoraux',
    back: 'Dos',
    shoulders: 'Épaules',
    biceps: 'Biceps',
    triceps: 'Triceps',
    quads: 'Quadriceps',
    hamstrings: 'Ischio-jambiers',
    glutes: 'Fessiers',
    calves: 'Mollets',
    abs: 'Abdos',
  },

  // ─── Exercise Row (shared component) ───
  exerciseRow: {
    restSuffix: 'repos',
    lastTime: 'Dernière fois',
    sets: 'Séries',
    reps: 'Répétitions',
    weight: 'Poids',
    rest: 'Repos',
    timePerSet: 'Temps/série',
    minReps: 'Min : %{min} reps',
    remove: 'Supprimer',
    reset: 'Réinitialiser',
  },

  // ─── Calendar Log (manual past workout) ───
  calendarLog: {
    title: 'Enregistrer une séance',
    workoutName: 'Nom de la séance',
    namePlaceholder: 'ex. Pecs + Triceps',
    duration: 'Durée',
    addExercise: 'Ajouter un exercice',
    sets: 'Séries',
    reps: 'Reps',
    weight: 'Poids',
    save: 'Enregistrer',
    noExercises: 'Ajoutez au moins un exercice',
    manualLabel: 'Manuel',
    logButton: 'Enregistrer une séance passée',
    dateLabel: 'Date',
    saved: 'Séance enregistrée',
  },

  // ─── Calendar Edit (session editor) ───
  calendarEdit: {
    title: 'Modifier la séance',
    setNumber: 'Série %{n}',
    addSet: 'Ajouter une série',
    removeExercise: 'Supprimer l\'exercice',
    addExercise: 'Ajouter un exercice',
    saveChanges: 'Enregistrer',
    unsavedTitle: 'Modifications non enregistrées',
    unsavedMessage: 'Abandonner les modifications ?',
    discard: 'Abandonner',
    exercisePreview: '+%{count} autres',
    sessionNotFound: 'Séance introuvable',
  },

  // ─── Auth ───
  auth: {
    // Welcome
    tagline: 'Entraîne-toi plus intelligemment',
    createAccount: 'Créer un compte',
    signIn: 'Se connecter',
    continueWithApple: 'Continuer avec Apple',

    // Sign Up
    signUpTitle: 'Crée ton compte',
    signUpSubtitle: 'Sauvegarde tes données dans le cloud',
    usernameLabel: 'Nom d\'utilisateur',
    usernamePlaceholder: 'ton_pseudo',
    usernameHint: 'Ton @pseudo unique — lettres, chiffres et _',
    usernameRequired: 'Un nom d\'utilisateur est requis',
    usernameTooShort: '3 caractères minimum',
    usernameInvalid: 'Lettres minuscules, chiffres et _ uniquement',
    emailLabel: 'E-mail',
    emailPlaceholder: 'ton@email.com',
    passwordLabel: 'Mot de passe',
    passwordPlaceholder: 'Minimum 6 caractères',
    passwordHint: '6 caractères minimum',
    createMyAccount: 'Créer mon compte',
    creatingAccount: 'Création du compte...',
    orDivider: 'ou',
    alreadyHaveAccount: 'Déjà un compte ?',
    signInLink: 'Se connecter',

    // Sign In
    signInTitle: 'Bon retour',
    signInSubtitle: 'Connecte-toi pour retrouver tes données',
    signInButton: 'Se connecter',
    signingIn: 'Connexion...',
    forgotPassword: 'Mot de passe oublié ?',
    noAccount: 'Pas encore de compte ?',
    createAccountLink: 'Créer un compte',

    // Forgot Password
    forgotTitle: 'Réinitialiser le mot de passe',
    forgotSubtitle: 'Entre ton adresse e-mail pour recevoir un lien de réinitialisation',
    sendResetLink: 'Envoyer le lien',
    sendingLink: 'Envoi en cours...',
    resetSentTitle: 'E-mail envoyé',
    resetSentMessage: 'Vérifie ta boîte mail pour réinitialiser ton mot de passe.',
    backToSignIn: 'Retour à la connexion',

    // Errors
    errorInvalidEmail: 'Adresse e-mail invalide',
    errorWeakPassword: 'Le mot de passe doit faire au moins 6 caractères',
    errorEmailInUse: 'Cette adresse e-mail est déjà utilisée',
    errorInvalidCredentials: 'E-mail ou mot de passe incorrect',
    errorNetworkError: 'Erreur réseau — vérifie ta connexion',
    errorGeneric: 'Une erreur est survenue. Réessaie.',

    // Settings — Account section
    accountSection: 'COMPTE',
    signedInAs: 'Connecté en tant que',
    syncStatus: 'Synchronisation',
    syncStatusSynced: 'À jour',
    syncStatusSyncing: 'En cours...',
    signOutButton: 'Se déconnecter',
    signOutConfirmTitle: 'Se déconnecter ?',
    signOutConfirmMsg: 'Tes données locales seront conservées. Tu pourras te reconnecter à tout moment.',
  },

  // ─── Profile Setup (Post-Signup Onboarding) ───
  profileSetup: {
    // Step 1: Identity
    identityTitle: 'Bienvenue',
    identitySubtitle: 'Fais connaissance avec ton app',
    nameLabel: 'Prénom',
    namePlaceholder: 'Thomas',
    nameHint: "C'est comme ça que l'app te saluera",
    emailConfirm: 'Compte créé avec %{email}',
    heyPreview: 'Hey, %{name}',
    // Step 2: Body
    bodyTitle: 'Ton physique',
    bodySubtitle: "Pour estimer tes charges et calibrer ton volume d'entraînement",
    sexLabel: 'Sexe',
    male: 'Homme',
    female: 'Femme',
    weightLabel: 'Poids',
    weightRequired: '(requis)',
    heightLabel: 'Taille',
    birthDateLabel: 'Date de naissance',
    birthDay: 'Jour',
    birthMonth: 'Mois',
    birthYearLabel: 'Année',
    agePreview: '%{age} ans',
    optionalHint: '(optionnel)',
    // Step 3: Experience
    experienceTitle: 'Ton niveau',
    experienceSubtitle: 'Pour adapter le volume et la difficulté',
    beginner: 'Débutant',
    beginnerDesc: "Moins d'1 an d'entraînement",
    intermediate: 'Intermédiaire',
    intermediateDesc: '1 à 3 ans régulier',
    advanced: 'Avancé',
    advancedDesc: '3+ ans sérieux',
    trainingYearsLabel: "Années d'entraînement",
    // Step 4: Week catch-up
    weekTitle: 'Ta semaine',
    weekSubtitle: "As-tu déjà fait des séances cette semaine ? L'app aura des données dès le départ.",
    restDay: 'Repos',
    duration: 'Durée',
    intensity: 'Intensité',
    light: 'Léger',
    moderate: 'Modéré',
    hard: 'Intense',
    musclesWorked: 'Muscles travaillés',
    logged: 'Enregistré',
    configureLater: 'Configurer plus tard',
    // Phone
    phoneLabel: 'Téléphone',
    phonePlaceholder: '06 12 34 56 78',
    phoneHint: 'Optionnel — pour retrouver tes amis plus tard',
    selectCountry: 'Indicatif pays',
    searchCountry: 'Rechercher un pays...',
    // Birth date inputs
    birthDayPlaceholder: 'JJ',
    birthMonthPlaceholder: 'MM',
    birthYearPlaceholder: 'AAAA',
    // Step 4
    tapToLog: 'Appuie sur un jour pour enregistrer ta séance',
    loggedSessions: '%{count} séance(s) enregistrée(s)',
    noSessionsYet: 'Aucune séance cette semaine',
    // Avatar
    addPhoto: 'Ajouter une photo',
    // Step 3b: Training setup
    goalTitle: 'Ton objectif',
    goalSubtitle: 'Pour adapter ton programme et tes recommandations',
    goalHypertrophy: 'Hypertrophie',
    goalHypertrophyDesc: 'Prendre du muscle, volume optimal',
    goalStrength: 'Force',
    goalStrengthDesc: 'Soulever plus lourd, intensité haute',
    goalRecomp: 'Recomposition',
    goalRecompDesc: 'Perdre du gras et gagner du muscle',
    equipmentTitle: 'Ton équipement',
    equipmentSubtitle: 'Pour choisir les bons exercices',
    equipmentFullGym: 'Salle complète',
    equipmentFullGymDesc: 'Barres, machines, câbles, haltères',
    equipmentHomeDumbbell: 'Maison + haltères',
    equipmentHomeDumbbellDesc: 'Haltères, élastiques, banc',
    equipmentBodyweight: 'Poids du corps',
    equipmentBodyweightDesc: 'Aucun matériel, exercices au sol',
    daysPerWeekTitle: 'Jours par semaine',
    daysPerWeekSubtitle: 'Combien de séances par semaine ?',
    // Nav
    skip: 'Passer',
    next: 'Continuer',
    finish: "C'est parti",
    stepOf: '%{current} sur %{total}',
  },

  // ─── Onboarding Slides (First App Open) ───
  onboarding: {
    slide1Title: 'Tes muscles, cartographiés',
    slide1Subtitle: 'Visualise chaque groupe musculaire et son état de récupération',
    slide2Title: 'Entraîne-toi dans la zone',
    slide2Subtitle: 'Volume optimal par muscle — basé sur la science de l\'hypertrophie',
    slide3Title: 'Progresse à chaque séance',
    slide3Subtitle: 'Surcharge progressive intelligente et records personnels automatiques',
    slide4Title: 'Récupère intelligemment',
    slide4Subtitle: 'Score de fatigue, détection de deload et fréquence optimale',
    continue: 'Continuer',
    getStarted: 'Commencer',
    skip: 'Passer',
  },

  // ─── Placeholders ───
  placeholders: {
    comingSoon: 'Bientôt disponible',
  },
};
